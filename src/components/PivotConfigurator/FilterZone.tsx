    ...
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        {filters.map(f => (
          <div
            key={f.field}
            className="bg-elevated border border-border rounded-[var(--radius-md)] px-2.5 py-2 flex flex-col gap-2"
          >
            <div className="flex items-center gap-1.5">
              <span className={[
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const next = selected
                          ...
                          : [...current, v]
                        onUpdate(f.field, { selectedValues: next })
                      }}
                      className={[
              ...
              <div className="flex items-center gap-2">
                <div className="flex flex-col gap-0.5 flex-1">
                  ...
                  <input
                    type="number"
                    value={f.min ?? ''}
                    onChange={e => onUpdate(f.field, { min: e.target.value === '' ? '' : Number(e.target.value) })}
                    placeholder="—"
                    className={[
                ...
                <div className="flex flex-col gap-0.5 flex-1">
                  ...
                  <input
                    type="number"
                    value={f.max ?? ''}
                    onChange={e => onUpdate(f.field, { max: e.target.value === '' ? '' : Number(e.target.value) })}
                    placeholder="—"
                    className={[
                      'w-full bg-surface border border-border rounded-[var(--radius-sm)]',
                      'px-2 py-1 text-xs text-text placeholder:text-subtle',
